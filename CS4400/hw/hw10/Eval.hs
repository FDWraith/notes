...
eval env genv (Cons e1 e2) = do
    List v <- eval env genv e2
    e' <- eval env genv e1
    return $ List (e' : v)
eval env genv Nil = do
    return $ List []
...
base = fromList 
    ...
    , mkPrim $ mkUnOp "list?" listHuh
    , mkPrim $ mkUnOp "nil?" nilHuh
    , mkPrim $ mkUnOp "cons?" consHuh
    , mkPrim $ mkUnOp "head" head
    , mkPrim $ mkUnOp "tail" tail
    ]
  where
    ...
    head (List (e:es)) = return e
    head _ = fail "head: expected a non-empty list"

    tail (List (e:es)) = return $ List es
    tail _ = fail "tail: expected a non-empty list"
    ...
    listHuh (List _) = return $ Boolean True
    listHuh _ = return $ Boolean False

    consHuh (List (e1:es)) = return $ Boolean True
    consHuh _ = return $ Boolean False

    nilHuh (List []) = return $ Boolean True
    nilHuh _ = return $ Boolean False
...
test_eval = do
    ...
    test "eval nil"
        (eval' $ Nil)
        (Success $ List [])
    test "eval (cons 1 (cons 2 nil))"
        (eval' $ (Cons (integer 1) (Cons (integer 2) Nil)))
        (Success $ List [Integer 1, Integer 2])
    test "eval (head (cons 1 (cons 2 nil)))"
        (eval' $ lhead (Cons (integer 1) (Cons (integer 2) Nil)))
        (Success $ Integer 1)
    test "eval (tail (cons 1 (cons 2 nil)))"
        (eval' $ ltail (Cons (integer 1) (Cons (integer 2) Nil)))
        (Success $ List [Integer 2])
    test "eval (tail nil)"
        (eval' $ ltail Nil)
        (Failure $ "tail: expected a non-empty list")
    test "eval (head nil)"
        (eval' $ lhead Nil)
        (Failure $ "head: expected a non-empty list")
    test "eval (list? nil)"
        (eval' $ (Call (Var "list?") [Nil]))
        (Success $ Boolean True)
    test "eval (cons? nil)"
        (eval' $ (Call (Var "cons?") [Nil]))
        (Success $ Boolean False)
    test "eval (nil? (tail (cons 1 nil)))"
        (eval' $ (Call (Var "nil?") [ltail (Cons (integer 1) Nil)]))
        (Success $ Boolean True)
  where
    ...
    lhead e = Call (Var "head") [e]
    ltail e = Call (Var "tail") [e]
