...
evalProgram (Program globals expr) = do
    ...
    eval fenv genv expr
  where
    evalGlobals :: (FunEnv, GlobalEnv) -> [GlobalDef] -> Maybe (FunEnv, GlobalEnv)
    evalGlobals envs (def : globals) = do
        envs' <- evalGlobal envs def
        evalGlobals envs' globals
    evalGlobals envs [] = return envs

-- |Evaluate a global in the given environments, returning updated environment
evalGlobal :: (FunEnv, GlobalEnv) -> GlobalDef -> Maybe (FunEnv, GlobalEnv)
evalGlobal (_, genv) (Define x _) | x `elem` keys genv = Nothing -- prohibit redefining variables
evalGlobal (fenv, genv) (Define x expr) = do
    v <- eval fenv genv expr
    return (fenv, set genv x v)
evalGlobal (fenv, _) (Defun f _ _) | f `elem` keys fenv = Nothing
...
eval fenv genv (Lt e1 e2) = 
    liftM2 (liftOrd (<) (<)) (eval fenv genv e1) (eval fenv genv e2)
...
evalArith intOp _ (Integer i1) (Integer i2) = Just $ Integer $ i1 `intOp` i2
evalArith _ fltOp (Integer i1) (Float f2) = Just $ Float $ fromIntegral i1 `fltOp` f2
evalArith _ fltOp (Float f1) (Integer i2) = Just $ Float $ f1 `fltOp` fromIntegral i2
evalArith _ fltOp (Float f1) (Float f2) = Just $ Float $ f1 `fltOp` f2
...
divide v1 v2 = liftM2 liftedDiv v1 v2
    where liftedDiv :: Value -> Value -> Maybe Value
...
runSExpression se =
    case fromSExpression se >>= eval empty empty of
