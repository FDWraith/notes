...
-- |Collects the global definitions
env :: GlobalEnv -> [GlobalDef] -> Maybe GlobalEnv
env m [] = Just m
env m (gdf:defs) =
  case gdf of
    (Define v ex) ->
      case get m v of
        Just _ -> Nothing
        _ -> env (set m v (Define v ex)) defs
    (Defun f args ex) ->
      case get m f of
        Just _ -> Nothing
        _ -> env (set m f (Defun f args ex)) defs

-- |Evaluates the final expression in a Program, taking into context the global definitions
evalProgram :: Program -> Maybe Value
evalProgram (Program globals ex) =
  case (env empty globals) of
    Just ev -> eval ev ex
    Nothing -> Nothing    
