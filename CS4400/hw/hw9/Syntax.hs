...
globalFromSExpr sig (S.List [ S.Symbol "defun", S.Symbol fname, S.List args, body]) 
    | not $ null args = do
        ...
        argTypes <- align fType (map unSymbol args)
        body' <- fromSExpression body
        return $ Define (TySig fname fType) (Lam argTypes body')
  where
...
fromKeyword s [S.List vars, e] | s `elem` ["lambda", "lam", "λ"] =
    Lam <$> mapM fromSigSExpression vars <*> fromSExpression e    
...
fromSigSExpression (S.List [ S.Symbol name, S.Symbol ":", typeSExpr ]) = do
      ty <- T.fromSExpression typeSExpr
...
toSExpression (Lam vars body) = S.List [ S.Symbol "lambda", S.List (map asSig vars), toSExpression body ]
  where
...
test_toSExpression = do
    ...
    test "toSExpression (lambda (x) x)"
      (toSExpression $ Lam [TySig "x" (TyBase TyInteger)] (Var "x"))
      (S.List [ S.Symbol "lambda"
              , S.List [ S.List [ S.Symbol "x", S.Symbol ":", S.Symbol "Integer" ] ] 
              , S.Symbol "x"])
